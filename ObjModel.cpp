...
void ObjModel::moveObject(int x, int y, int z) {
	this->position = this->position + glm::vec3(x, y, z);
}

void ObjModel::resize(double facX, double facY, double facZ)
{
	for (GLuint i = this->_vertexArray.begin(); i != this->_vertexArray.end(); i++) {
...
ObjModel::ObjModel(std::string fileName, glm::vec3 position)
{
