...
void init()
{
	...
	regularShaders.push_back(loadShader("diffuse.vs", "diffuse.fs"));
	...
	//postShaders.push_back(loadShader("toonshader.vs", "toonshader.fs"));


	models.push_back(new ObjModel("models/ship/shipA_OBJ.obj", glm::vec3(0, 0, 150)));
	models.push_back(new ObjModel("models/car/honda_jazz.obj", glm::vec3(0, 0, 50)));
...
void display()
{
	...
	glm::mat4 projection = glm::perspective(90.0f, screenSize.x / (float)screenSize.y, 0.01f, 300.0f);		//begin met een perspective matrix
	glm::mat4 view = glm::lookAt(models[activeModel]->getPosition(), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));					//vermenigvuldig met een lookat
	glm::mat4 model = glm::translate(glm::mat4(), camera);													//of verplaats de camera gewoon naar achter
	model = glm::rotate(model, rotation, glm::vec3(0, 1, 0));

	//roteer het object een beetje
...
void keyboard(unsigned char key, int x, int y)
{
	...
	switch (key) {
		case 'i': currentShader = (currentShader + regularShaders.size() - 1) % regularShaders.size(); 
			break;
		case 'o': currentShader = (currentShader + 1) % regularShaders.size(); 
			break;
		case 'k': currentPostShader = (currentPostShader + postShaders.size() - 1) % postShaders.size(); 
			break;
		case 'l': currentPostShader = (currentPostShader + 1) % postShaders.size(); 
			break;
		case ',': activeModel = (activeModel + 1) % models.size(); 
			break;
		case 'w': camera = camera + glm::vec3(0, 0, speed);
			break;
		case 's': camera = camera + glm::vec3(0, 0, -speed);
			break;
		case 'a': camera = camera + glm::vec3(-speed, 0, 0);
			break;
		case 'd': camera = camera + glm::vec3(speed, 0, 0);
			break;
		case 'f': camera = camera + glm::vec3(0, speed, 0);
			break;
		case 'r': camera = camera + glm::vec3(0, -speed, 0);
			break;
		case VK_ESCAPE: glutLeaveMainLoop(); 
			break;
	}
}
